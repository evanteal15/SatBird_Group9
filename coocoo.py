...
def filter_by_sat():
    ...
    summer = pd.read_csv("./SatBird_data_complete/all_hotspots_cleaned.csv")

    hotspots = [r.strip(".npy") for r in os.listdir("./SatBird_data_complete/environmental")]

    uu = summer[summer["hotspot_id"].isin(hotspots)]

    uu.drop(columns = ["Unnamed: 0"]).to_csv("./SatBird_data_complete/all_hotspots_cleaned2.csv")
